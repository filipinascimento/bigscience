...
with open(dataPath/"Biomedical_MAGID2Title.json","rt") as fd:
    allProjectsID2Title = json.load(fd)
...
ID2Title = {int(magID):title for magID,title in allProjectsID2Title.items()}
...
for projectName in ["HGP","HuBMAP+HCA"]:
  for edgesType in ["Referenced","Citing"]:
    edgesTable = pd.read_csv(dataPath/"ToMapOfScience"/("Biomedical_Edges_%s_%s_tomap.csv.gz"%(edgesType,projectName)))
    ...
    edgesTable.to_csv(dataPath/"ToMapOfScience"/("Biomedical_Edges_%s_%s_tomap_keywords.csv.gz"%(edgesType,projectName)))
...
for projectName in ["HGP","HuBMAP+HCA"]:
  ...
  for edgesType in ["Referenced","Citing"]:
    edgesTable = pd.read_csv(dataPath/"ToMapOfScience"/("Biomedical_Edges_%s_%s_tomap_keywords.csv.gz"%(edgesType,projectName)))
...
projectMapCoverageTable.to_csv(dataPath/"ToMapOfScience"/("Biomedical_projectMapCoverageTable.csv"))
